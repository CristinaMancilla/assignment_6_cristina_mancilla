...
```
Using Pearson's Chi-square test to test for significance, I found a significant association (p<0.05) between political affiliation and self - identification as an environmentalist ($Chi$^2^( `r envipo_tidy$parameter` ) = `r envipo_tidy$statistic`, *p* = `r envipo_tidy$p.value `). Many respondents who identified as identified as "conservative" did not identify as an "environmentalist" (71.3%). Only 28% of "liberals" also responded "No". Athough the difference is not as drastic, more "liberals" responded "Yes" (56.88%) identifying as an "environmentalist" than "conservatives" (43.12%).  
